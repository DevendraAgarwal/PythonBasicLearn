...
Slicing:
    You can extract a subset of characters from a string using slice notation,
    e.g., s[1:3] returns the characters at indices 1 and 2.

Concatenation:
    Strings can be concatenated using the + operator or the str.format() method.

Methods:
    Python strings have various built-in methods, such as:
...
Raw Strings:
    Raw strings are prefixed with an r and treat backslashes (\) as 
    literal characters, rather than escape characters.

Unicode:
    Python 3.x uses Unicode strings by default, which support characters 
